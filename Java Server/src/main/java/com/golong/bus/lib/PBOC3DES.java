...
public class PBOC3DES {
	...
	public static void main(String[] args) {
		...
		String result = PBOC_3DES_MAC("2000000000000000000000510001000001504F533838383834343939313330323933393930303037","4338EA140CF44A07BE2C840267A0EF4A",HEX);
...
//			System.out.println("step2--->" + temp);
	 ...
	 */
	 ...
	 public static byte[] PBOC_DES_MAC_D(byte[] shuju, byte[] key, int bFlag)
     {
		 try {
	         ...
	         do
	         {
	             ...
	             {
	                 try {
						bytTmpX = SDecrypt_DES(bytTmp, key);
					} catch (Exception e) {
						e.printStackTrace();
					}
	             }
	             ...
	             {
	                 try {
						bytTmpX = SEncrypt_DES(bytTmp, key);
					} catch (Exception e) {
						e.printStackTrace();
					}
	             }
	 ...
	 public static byte[] SEncrypt_DES(byte[] datasource, byte[] password) {              
	        try{  
	 ...
	 private static byte[] SDecrypt_DES(byte[] src, byte[] password) throws Exception {  
	        // DES算法要求有一个可信任的随机数源  
	        SecureRandom random = new SecureRandom();  
	        // 创建一个DESKeySpec对象  
	        DESKeySpec desKey = new DESKeySpec(password);  
